...
public class BaseIntegrationTest {
	...
	public AccessTokenAndRefreshToken signupAndLoginNoAddressMember() throws Exception {
		...
		// 로그인
		LoginMemberRequest loginMemberRequest = new LoginMemberRequest(NO_ADDRESS_MEMBER_EMAIL, NO_ADDRESS_MEMBER_PW);
	...
	public AccessTokenAndRefreshToken signupAndLoginAddressMember() throws Exception {

		// 멤버 저장
		Member member = Member.createWithAddress(
		...
		// 로그인
		LoginMemberRequest loginMemberRequest = new LoginMemberRequest(ADDRESS_MEMBER_EMAIL, ADDRESS_MEMBER_PW);
		ResultActions resultActions = mockMvc.perform(post("/api/members/login")
			.contentType(MediaType.APPLICATION_FORM_URLENCODED_VALUE)
			.param("email", loginMemberRequest.email())
			.param("pw", loginMemberRequest.pw())
